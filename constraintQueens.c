...
int isBitmaskSet(int col, int row) {
    ...
    // Die Bitmaske besteht nun aus n 32-Bit Werten
    // In welchem 32-Bit Wert steht das Bit für die Zeile?
    div_t result = div((row -1),32);
    int idx = result.quot;
    return(bms[idx] & (1 << result.rem));
}

int testGetBitmask(int col) {
    // Für die Tests reicht es, das erste 32-Bit Wort zurückzugeben
    cba(attackBitMap, col, __LINE__);
    return attackBitMap[col][0];
}
...
int setBitmask(int col, int row) {
    ...
    div_t result = div((row -1),32);
    int idx = result.quot;
    int mask = (1 << result.rem);

    if(bms[idx] & mask) {
        // das Feld ist bereits markiert als bedroht
        return -1;
    }
    else {
        cba(attackBitMap, col, __LINE__);
...
void resetColumns(col) {
    int faktor = 1;

#ifdef DEBUG
    printf("in resetColumns: col=%d\n", col);
#endif

    columns[col] = 0;

    for(int c=1; c<=colCount; c++) {
        freeFieldsCount[c] = colCount;
    ...
    for(int c=1; c<=colCount; c++) {
        if(columns[c] > 0) {
            ...
            for(int i=1; i<=colCount; i++) {
                if(columns[i] != 0 ) { // In der Spalte steht schon eine Dame
                    continue;
                }
                // Die gleiche Reihe ist nicht mehr frei
                setBitmask(i, row);
...
setNextFreeColumns(int col, int *freeCols, int *nextFreeCols) {
#ifdef DEBUG
...
void init() {
    ...
    for (int i = 0; i < colCount+1; i++) {
        attackBitMap[i] = (int *)malloc(BITMAP_SIZE * sizeof(int));
        for (int j = 0; j < BITMAP_SIZE; j++) {
            attackBitMap[i][j] = 0;
        }
    }
...
void constraintQueens(int n) {
    ...
    if(doTests) {
        ...
        for(int i=1; i<=freeColumns[2][0]; i++) {
            ...
            printf("getBitmask(%d): %d\n", freeColumns[2][i], testGetBitmask(freeColumns[2][i]));
        }
        fflush(stdout); // Erzwingt die Ausgabe des Puffers
        ...
        for(int i=1; i<=freeColumns[3][0]; i++) {
            ...
            printf("getBitmask(%d): %d\n", freeColumns[3][i], testGetBitmask(freeColumns[3][i]));
        }
        resetColumns(2);
        ...
        for(int i=1; i<=freeColumns[1][0]; i++) {
            ...
            printf("getBitmask(%d): %d\n", freeColumns[1][i], testGetBitmask(freeColumns[1][i]));
        }
        printf("----------------\n");
        fflush(stdout); // Erzwingt die Ausgabe des Puffers
